...
class State(CybORGLogger):
    ...
    def __init__(self, scenario: Scenario, np_random):
        ...
        # contains mapping of agent names to mapping of session id to session objects
        self.sessions = None
    ...
    def get_true_state(self, info: dict) -> Observation:
        ...
        if info is None:
            raise ValueError(
                'None is not a valid argument for the get true state function in the State class')
        else:
                ...
                if hostname in info:
                    ...
                    if 'Interfaces' in info[hostname]:
                        if info[hostname]['Interfaces'] == 'All':
                            for interface in host.interfaces:
                                true_obs.add_interface_info(
                                    hostid=hostname, **interface.get_state())
                        elif info[hostname]['Interfaces'] == 'IP Address':
                            for interface in host.interfaces:
                                if interface.name != 'lo':
                                    true_obs.add_interface_info(
                                        hostid=hostname, ip_address=interface.ip_address)
                        else:
                            raise NotImplementedError(
                                f"{info[hostname]['Interfaces']} cannot be collected from state")
                    ...
                    if 'Files' in info[hostname]:
                        for file in host.files:
                            true_obs.add_file_info(
                                hostid=hostname, **file.get_state())
                    ...
                    if 'System info' in info[hostname]:
                        true_obs.add_system_info(
                            hostid=hostname, **host.get_state())

                    if 'Services' in info[hostname]:
                        if 'All' in info[hostname]['Services']:
                            for service, service_info in host.services.items():
                                true_obs.add_process(
                                    hostid=hostname, service_name=service, pid=service_info['process'])
                        else:
                            for service_name in info[hostname]['Services']:
                                if service_name in host.services:
                                    true_obs.add_process(
                                        hostid=hostname, service_name=service_name, pid=host.services[service_name]['process'])
        return true_obs
            ...
            for starting_session in agent_info.starting_sessions:
                if starting_session.parent is not None:
                    ...
                    self.sessions[agent][len(self.sessions[agent])] = self.hosts[starting_session.hostname].add_session(
                        ...
                        name=starting_session.name,
                        artifacts=starting_session.event_artifacts)
                    parent.children[self.sessions_count[agent]
                                    ] = self.sessions[agent][self.sessions_count[agent]]
                    self.sessions_count[agent] += 1
    ...
    def update_data_links(self):
        distances = {hostname: {hostname: 0.}
                     for hostname in self.hosts.keys()}
        for hostname, host_info in self.hosts.items():
            for hostname2, host_info2 in self.hosts.items():
                if hostname2 not in distances[hostname]:
                    distances[hostname][hostname2] = self.dist(
                        host_info.position, host_info2.position)
        ...
        for hostname, host_info in self.hosts.items():
            for interface in host_info.interfaces:
                if interface.interface_type != 'wired':
                    ...
                    interface.data_links = [other_hostname for other_hostname, other_host_info in self.hosts.items(
        ...
        self.connected_components = [
            i for i in connected_components(self.link_diagram)]
    ...
    def remove_process(self, hostname: str, pid: int):
        ...
        if process is not None:
            agent, session = self.get_session_from_pid(
