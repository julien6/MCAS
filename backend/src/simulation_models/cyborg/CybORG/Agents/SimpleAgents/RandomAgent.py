    ...
    def get_action(self, observation: dict, action_space: Union[Space, dict]):
        if (self.np_random.random() < self.epsilon) or (self.test_action is None):
            ...
            if isinstance(action_space, Space):
                ...
                invalid_actions = []
                while True:
                    options = [i for i, v in action_space['action'].items(
                    ) if v and i not in invalid_actions]
                    if len(options) > 0:
                        action_class = self.np_random.choice(options)
                    else:
                        return Sleep()
                    # select random options
                    self.set_initial_values(action_space, None)
                    action_params = {}
                    for param_name in self.action_params[action_class]:
                        options = [
                            i for i, v in action_space[param_name].items() if v]
                        if len(options) > 0:
                            action_params[param_name] = self.np_random.choice(
                                options)
                        else:
                ...
                raise ValueError(
                    "Random agent can only handle Space or dict action space")
        else:
